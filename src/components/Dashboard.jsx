  ...
  return (
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      ...
      <span style={{
        ...
        fontFamily: 'Fraunces, serif', fontSize: 18, fontWeight: 600, color: C.primary,
      }}>
        {done}
      </span>
    </div>
  ...
  return (
    <div style={{ textAlign: 'center', flexShrink: 0 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
        ...
        {active && (
          <span style={{ fontFamily: 'Fraunces, serif', fontSize: 20, fontWeight: 600, color: C.gold }}>
            {streak}
          </span>
        )}
      </div>
    ...
    >
      ...
      <span style={{ flex: 1, minWidth: 0 }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontFamily: 'Fraunces, serif', fontSize: 16.5, fontWeight: 600, color: C.text }}>
            {tool.name}
          </span>
          {tool.badge && <span style={pill(tool.badgeColor)}>{tool.badge}</span>}
        </span>
        <span style={{ display: 'block', fontSize: 12.5, color: C.textSoft, marginTop: 2 }}>{tool.desc}</span>
      </span>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {todayTotals.due > 0 ? (
            <div style={{ fontSize: 14 }}>
              <span style={{ fontFamily: 'Fraunces, serif', fontSize: 22, fontWeight: 600, color: C.primary }}>
                {todayTotals.due}
              </span>
              {' '}Karten heute fällig
              {todayTotals.fresh > 0 && (
          ...
          <div style={{ fontSize: 12, color: C.textSoft, marginTop: 3 }}>{doneToday} heute wiederholt</div>
        </div>
        <StreakBadge streak={streak} />
      </div>

      <div style={{ fontSize: 12.5, fontWeight: 500, color: C.textSoft, marginBottom: '0.6rem' }}>Lernwerkzeuge</div>
